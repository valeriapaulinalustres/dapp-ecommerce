...
const IMAGE =
  "https://cdn.pixabay.com/photo/2013/07/12/18/20/shoes-153310_640.png"; //se hace con ipfs, ver tutorial
...
describe("Dappecommerce", () => {
  ...
  describe("Listing", () => {
    ...
    beforeEach(async () => {
      //List an item
      transaction = await dappecommerce
        .connect(deployer)
  ...
  describe("Withdrawing", () => {
    ...
    beforeEach(async () => {
      ...
      //Withdraw
      transaction = await dappecommerce.connect(deployer).withdraw()
      await transaction.wait();

    });
      
it("Updates de owner balance", async ()=>{
const balanceAfter = await ethers.provider.getBalance(deployer.address)
expect(balanceAfter).to.be.greaterThan(balanceBefore)
})

it("Updates the contract balance", async ()=>{
  const address = await dappecommerce.getAddress(); 
  const result = await ethers.provider.getBalance(address)
